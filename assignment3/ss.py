...
class StopAndWait:
    ...
    def wait_for_ack(self, sequence):
        ...
        if (time.time() - self._time) >= config.TIMEOUT_MSEC:
            ...
            return False
                ...
                if (util.check_corrupted(self.received_packet) or not (util.is_ack(self.received_packet, seq_num))):
                    return False
                else:
                    #clear the time
                    self._time = None
    ...
    def handle_arrival_msg(self):
        ...
        if self.role == "receiver":
            if util.check_corrupted(message) or util.get_seq(message) != self.current_state:
    ...
    def shutdown(self):
        ...
        # class.
        while self.current_state !=0 and self.current_state !=2:
