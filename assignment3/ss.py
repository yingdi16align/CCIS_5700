...
class StopAndWait:
    ...
    def wait_for_ack(self, sequence):
        ...
        if time.time() - self._time > config.TIMEOUT_MSEC:
            ...
            return False
                ...
                if util.is_message_corrupted(self.received_packet) or not (util.check_ack(self.received_packet, sequence)):
                    return False
                else:
                    #clear the time
                    self._time = None
    ...
    def handle_arrival_msg(self):
        ...
        if self.role == "receiver":
            if util.is_message_corrupted(message) or (struct.unpack("!H", message[2:4])[0])!= self.current_state:
