...
class GoBackN:
    ...
    def send(self, msg):
        ...
        if self._time and (time.time() - self._time > config.TIMEOUT_MSEC/1000):
            next_id=self.get_next_id()
            self._time = time.time()
            for packet in self.packets_wait_sending[(self.current_id % self.window_size):next_id]:
    ...
    def handle_arrival_msg(self):
        ...
        if self.role =="sender":
            if (struct.unpack("!H", msg[0:2])[0])==config.MSG_TYPE_ACK:
            ...
            if (struct.unpack("!H",msg[2:4])[0]) == self.required_seq:
